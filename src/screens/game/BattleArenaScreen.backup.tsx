...
import { StyleSheet, View, TouchableOpacity, Image, Animated, Dimensions, ScrollView, ImageBackground, Alert } from 'react-native'
import { ThemedText } from '@/components'
import { Panel, LoadingContainer, CustomAlert } from '@/components/ui'
...
export const BattleArenaScreen: React.FC = () => {
  ...
  useEffect(() => {
    const initBattle = async () => {
      if (params.playerPet && params.opponent && !battleState) {
        ...
        // Ensure moves exist, use defaults if not
  ...
  const completeBattleSession = async (won: boolean, finalHp: number) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Store battle result for rewards dialog
        setBattleResult({
  ...
  return (
    ...
    >
      ...
      {/* Battle Rewards Dialog - Shows detailed rewards with level up info */}
      <CustomAlert
  ...
  // Helper function to build rewards message
  function buildRewardsMessage(result: typeof battleResult): string {
    if (!result) return ''
